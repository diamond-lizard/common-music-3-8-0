...
void Console::setTheme(int index)
{
  ...
  buffer->setColour( CaretComponent::caretColourId, theme->getColor(ConsoleTheme::caretColor));
  ...
  for (int i=ConsoleTheme::bgColor; i<=ConsoleTheme::caretColor; i++)
    {
      ...
      if (c1!=c2) std::cout << th->getName() << "(" << i << ") failed! orig=" << c1.toString() << ", html="<< st << ", back=" << c2.toString() << "\n" ;
    }
}
