...
public:
  ...
  FOMUSSettingsEditor(fomusinfo* inf, FOMUSListBoxModel& boxmodel, FOMUSSettingsListBoxItem& lb, const String &componentName = String::empty,
		      const juce_wchar passwordCharacter = 0):TextEditor(componentName, passwordCharacter), inf(inf), boxmodel(boxmodel), lb(lb), dirty(false), invalid(false) {}
...
void FomusSyntax::stickkeyword(String str, const int hl) 
{
  ...
  addSynTok(str.toUpperCase(), hl);
}

StringArray FomusSyntax::getTokenTypes () 
{ 
